  ...
  try {
    ...
    if (citiesRes.ok) {
      ...
      for (const city of citiesJson.data || []) {
        ...
        urls.push(
          `${SITE_URL}/kode-pos/${provinsi}/${citySlug}/`
        );
        ...
        for (const district of districtsJson.data || []) {
          ...
          urls.push(
            `${SITE_URL}/kode-pos/${provinsi}/${citySlug}/${districtSlug}/`
          );
        }
      }
    }
    ...
    console.error(
      "KODE POS SITEMAP ERROR:",
      error
    );

    return new Response(
      "Sitemap Kode Pos gagal dibuat",
      {
        status: 500,
      }
    );
